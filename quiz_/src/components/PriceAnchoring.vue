...
<template>
  <div class="bg-orange-50 p-6 rounded-lg border-2 border-orange-200">
    ...
    <div class="flex items-center justify-between mb-4">
      <div class="w-20 h-20 rounded-lg overflow-hidden border-2 border-orange-200">
        <img
    ...
    <div class="space-y-2 mb-6">
      ...
      <div class="flex justify-between text-xl font-bold">
        ...
        <span class="text-orange-600">${{ finalPrice.toFixed(2) }}</span>
      </div>
    </div>

    <!-- Bonus items section - now integrated in the same block -->
    <div v-if="showBonusItems" class="border-t-2 border-orange-200 pt-6">
      <h3 class="text-2xl font-bold text-orange-900 mb-6 text-center">What's included in your jersey? 🏀⚡</h3>
        ...
        <div
          ...
          :class="`flex items-center justify-between p-4 rounded-lg transition-all duration-500 ${
            visibleItems.includes(index)
              ? 'opacity-100 transform translate-x-0 bg-white border border-orange-200 shadow-sm'
