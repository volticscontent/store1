...
/**
 * FUNÇÃO TRACK COM EVENTOS P2
 */
export const trackEvent = (eventName: string) => {
  try {
    const p2Event = `P2:${eventName}`
    console.log('🎯 P2 EVENT:', p2Event)
    
    // Facebook - evento direto P2
    if (window.fbq) {
      window.fbq('track', p2Event)
      console.log('📘 FB P2:', p2Event)
    } else {
      console.warn('📘 FB não carregado')
    }
    
    // TikTok - mapeia P2 para eventos aceitos
    if (window.ttq) {
      const tiktokEvent = tiktokMap[p2Event] || 'ClickButton'
      window.ttq.track(tiktokEvent, getTikTokParams(eventName))
      console.log('📱 TT P2:', p2Event, '→', tiktokEvent)
    } else {
    ...
    // Google Ads
    if (window.gtag) {
      window.gtag('event', p2Event, { send_to: 'AW-10873800335' })
      console.log('🔍 GA P2:', p2Event)
    }
  } catch (error) {
    console.error('❌ Erro no tracking P2:', error)
  }
}

/**
 * FUNÇÃO TRACK - MANTIDA PARA COMPATIBILIDADE
 */
